angular.module('mindmapApp').directive('ngMindmap', function(){
    return {
        ...
        templateUrl: 'app/shared/mindmap/mindmap.vw.html',
        css: 'app/shared/mindmap/mindmap.stl.css',
        controller: function($scope, $element, $rootScope){
            ...
            Node = function (obj, name, parent, opts) {
                ...
                this.el = $('<a href="' + this.href + '"><div class="label">'+ this.name +'</div></a>').addClass('node');
                ...
                this.el.draggable({ containment: $($element), scroll: false });
            ...
            Node.prototype.getForceVector = function () {
                ...
                if (this.obj.activeNode === this) {
                    ...
                    x1 = (($($element).width() / 2) - this.x);
                    y1 = (($($element).height() / 2) - this.y);
                    dist = Math.sqrt((x1 * x1) + (y1 * y1));
            ...
            $.fn.mindmap = function (options) {
                ...
                return this.each(function () {
                    ...
                    if (options.mapArea.x === -1) {
                        ...
                        options.mapArea.x = $($element).width();
                    }
                    if (options.mapArea.y === -1) {
                        //options.mapArea.y = $window.height();
                        options.mapArea.y = $($element).height();
                    }
                    //create drawing area
                    this.canvas = Raphael(13.5, 13, options.mapArea.x, options.mapArea.y);
        ...
        link: function(scope, element, attr){
            // enable the mindmap in the area
            $(element).mindmap();

            var rootLi = $(element).find('li').get(0);
            var rootA = $(rootLi).find('a').get(0);

            var root = rootLi.mynode = $(element).addRootNode($(rootA).text(), {
                href:'#/',
                url:'#/',
            ...
            var addLI = function() {
                ...
                this.mynode = $(element).addNode(parentnode, $('a:eq(0)',this).text(), {
            ...
            $(element).find('>ul>li>ul').each(function() {
                $('>li', this).each(addLI);
            });
        }
    };
